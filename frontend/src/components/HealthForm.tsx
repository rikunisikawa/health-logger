...
export default function HealthForm({
  ...
  >(() => {
    try {
      ...
      return stored
        ? {
            ...defaults,
            ...(JSON.parse(stored) as Partial<Record<SliderKey, boolean>>),
          }
        : defaults;
    } catch {
      return {
  ...
  const toggleStatus = async (itemId: string, label: string) => {
    ...
    const record: HealthRecordInput = {
      ...
      app_version: "1.0.0",
      custom_fields: [
        { item_id: itemId, label, type: "checkbox", value: nextActive },
      ],
    };
    const ok = await submitRecord(record);
    if (ok) {
  ...
  const handleQuickEvent = async (
    ...
    const record: HealthRecordInput = {
      ...
      app_version: "1.0.0",
      custom_fields: [
        { item_id: item.item_id, label: item.label, type: item.type, value },
      ],
    };
    const ok = await submitRecord(record);
    if (ok) {
  ...
  return (
    <div className="container py-3" style={{ maxWidth: "540px" }}>
      ...
      >
        ...
        {!isNowSelected && (
          <p
            className="mb-0 mt-1 text-warning-emphasis"
            style={{ fontSize: "0.72rem" }}
          >
            ...
            {[...STATUS_ITEMS, ...statusItems].map((item) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  className={
                    isOn ? "btn btn-warning" : "btn btn-outline-warning"
                  }
                ...
                [
                  {
                    key: "fatigue",
                    label: "疲労感",
                    color: SLIDER_COLORS.fatigue,
                  },
                  { key: "mood", label: "気分", color: SLIDER_COLORS.mood },
                  {
                    key: "motivation",
                    label: "やる気",
                    color: SLIDER_COLORS.motivation,
                  },
                  {
                    key: "concentration",
                    label: "集中力",
                    color: SLIDER_COLORS.concentration,
                  },
                ] as const
              ).map(({ key, label, color }) => (
                <label
                  ...
                  style={{
                    ...
                    backgroundColor: enabledSliders[key]
                      ? `${color}18`
                      : "#f8f9fa",
            ...
            {(
              [
                ...
                },
                {
                  ...
                  value: mood,
                  setter: setMood,
                  colorKey: "mood" as const,
                  prev: prevMood,
                },
                {
                  label: "やる気",
                  value: motivation,
                  setter: setMotivation,
                  colorKey: "motivation" as const,
                  prev: prevMotivation,
                },
                {
                  label: "集中力",
                  ...
                  colorKey: "concentration" as const,
                  prev: prevConcentration,
                },
              ] as const
            )
                    ...
                    {item.type === "checkbox" && (
                      <div className="form-check">
                        <input
                          ...
                          onChange={(e) =>
                            setCustomValue(item.item_id, e.target.checked)
                          }
                        />
                        <label
                          className="form-check-label"
                          htmlFor={`custom-${item.item_id}`}
                        >
                          {item.icon && (
                    ...
                    {item.type === "slider" && (
                      <div>
                        ...
                        <Slider
                          ...
                          onChange={(_, v) =>
                            setCustomValue(item.item_id, v as number)
                          }
                    ...
                    {item.type === "text" && (
                      <div>
                        ...
                        <input
                          ...
                          value={String(customValues[item.item_id] ?? "")}
                          onChange={(e) =>
                            setCustomValue(item.item_id, e.target.value)
                          }
                        />
                      </div>
                    )}
