...
const METRIC_CONFIG: Record<
  MetricKey,
  { name: string; stroke: string; btnVariant: string }
> = {
...
export default function DashboardPage({ onBack }: Props) {
  ...
  useEffect(() => {
    ...
    getLatest(token, 500)
      ...
      .catch((e: unknown) =>
        setFetchError((e as Error).message ?? "取得に失敗しました"),
      )
  ...
  useEffect(() => {
    ...
    getEnvData(token, days)
      .then((res) => setEnvRecords(res.records))
      .catch(() => {});
  ...
  const { intradayStatusPeriods, intradayStatusLabels } = useMemo((): {
    ...
    const dayRecords = records
      .filter(
        (r) =>
          r.record_type === "status" &&
          toLocalDateStr(parseUtc(r.recorded_at)) === selectedDate,
      )
      .sort(
        (a, b) =>
          parseUtc(a.recorded_at).getTime() - parseUtc(b.recorded_at).getTime(),
      );

    const statusByLabel: Record<
      string,
      { minutes: number; isStart: boolean }[]
    > = {};
    for (const r of dayRecords) {
      const minutes = toLocalMinutes(parseUtc(r.recorded_at));
      try {
        const fields = JSON.parse(r.custom_fields || "[]") as {
          label: string;
          type: string;
          value: unknown;
        }[];
  ...
  const xAxisOptions = useMemo(() => {
    ...
    const events = eventLabels.map((l) => ({
      value: `event:${l}`,
      label: `${l}（イベント）`,
    }));
    return [...fixed, ...events];
              ...
              (trendData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={280}>
                  ...
                  >
                    ...
                    <Tooltip />
                    <Legend />
                    {ALL_METRICS.filter((k) => visibleMetrics.has(k)).map(
                      (key) => (
                        <Line
                          key={key}
                          type="monotone"
                ...
                {(intradayData.length > 0 ||
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    ...
                    >
                      ...
                      <YAxis domain={[0, 100]} tick={{ fontSize: 11 }} />
                      <Tooltip
                        labelFormatter={(v) => minuteTick(v as number)}
                      />
                      <Legend />
                      ...
                      {intradayStatusPeriods.map((p, i) => {
                        ...
                        return (
                          <ReferenceArea
                            ...
                            x2={p.endMinutes}
                            fill={
                              STATUS_COLORS[colorIdx % STATUS_COLORS.length]
                            }
                            fillOpacity={0.22}
                            ifOverflow="hidden"
                          />
                  ...
                  <div className="d-flex flex-wrap gap-1 mt-1 mb-3">
                    ...
                    {intradayStatusLabels.map((label, i) => (
                      <span
                        ...
                        style={{
                          background:
                            STATUS_COLORS[i % STATUS_COLORS.length] + "33",
                          color: "#333",
                          fontSize: 11,
                    ...
                    >
                      ...
                      >
                        ...
                        <YAxis
                          ...
                          domain={[
                            -0.5,
                            Math.max(eventCategories.length - 0.5, 0.5),
                          ]}
              ...
              <>
                ...
                {eventTrendData.length === 0 ? (
                  ...
                  <div className="d-flex flex-column gap-1">
                    {/* パネル1: 体調スコア */}
                    <p className="mb-0 small text-muted fw-semibold">
                      体調スコア
                    </p>
            ...
            {!corrRegression &&
              ...
              corrPoints.length < 3 && (
                <p className="text-muted small mb-3">
                  散布点が {corrPoints.length}{" "}
                  件のため回帰直線を表示できません（3件以上必要）。
                </p>
              )}
              ...
              <ResponsiveContainer width="100%" height={320}>
                ...
                >
                  ...
                  <XAxis
                    ...
                    tick={{ fontSize: 11 }}
                    name={
                      xAxisOptions.find((o) => o.value === corrXAxis)?.label ??
                      corrXAxis
                    }
                    label={{
                      value:
                        xAxisOptions.find((o) => o.value === corrXAxis)
                          ?.label ?? corrXAxis,
                      position: "insideBottomRight",
                      offset: -4,
                      fontSize: 11,
